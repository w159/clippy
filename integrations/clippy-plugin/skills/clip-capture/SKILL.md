...
2. **Title it well.** Pass a short, specific `title` to `clippy_create_clip` that says what the clip does, not where it came from. Good: "ffmpeg trim first 10s", "Postgres connection string (staging)". Bad: "snippet", "from conversation".

3. **Categorize when obvious.** Call `clippy_list_categories` first. If an existing category clearly fits (for example "Commands", "URLs", "Snippets"), file the new clip there with `clippy_assign_clips` after adding it. Do not invent new categories for a single clip unless the user asks.

4. **Confirm.** Report the returned clip id and title. The clip appears in the running app within about two seconds.

## Rules

- One clip per logical artifact. If the user wants three commands saved, that is three `clippy_create_clip` calls with three titles, not one blob.
